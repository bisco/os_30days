...
void HariMain(void) {
    ...
    io_out8(PIC0_IMR, 0xf8); // PIT、PIC1、キーボードを許可
...
void console_task(struct SHEET *sheet) {
    ...
    for(;;) {
        ...
        if(fifo32_status(&task->fifo) == 0) {
            ...
            if(i == 3) { // カーソルOFF
                boxfill8(sheet->buf, sheet->bxsize, COL8_000000, cursor_x, 28, cursor_x + 7, cursor_y + 15);
                cursor_c = -1;
            }
            if(256 <= i && i <= 511) { // キーボードデータ(タスクA経由)
                if(i == 8 + 256) {
                    ...
                    if(cursor_x > 16) {
                        // カーソルをスペースで消してから、カーソルを1つ戻す
                        putfonts8_asc_sht(sheet, cursor_x, cursor_y, COL8_000000, COL8_000000, " ", 1);
                        cursor_x -= 8;
                    }
                } else if(i == 10 + 256) {
                    // enter
